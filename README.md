...
- **Right-click context menu** — right-clicking a node or edge opens a floating menu with instant actions and section-targeted sidebar redirects. In view mode: Duplicate, Copy node id (nodes only), Delete. In edit mode the menu expands with single-click mutations (Change type ▸ submenu of 17 node or 6 edge types, Compact mode toggle, edge Change type ▸, Anchor source ▸, Anchor target ▸, Flow speed ▸, Bidirectional and Flow animation toggles) and sidebar-redirect shortcuts (Edit metrics / Edit alert matchers / Edit observability links for nodes; Edit metric binding / Edit thresholds / Edit state map / Edit visual for edges) that open the matching sidebar card already scrolled to the right sub-section. Depth-2 submenus open to the right (flipping left on overflow). Full keyboard navigation: ArrowUp/Down cycles items, ArrowRight opens a submenu, ArrowLeft or Escape closes the submenu, Escape at the root closes the whole menu. Clamps to panel bounds.
